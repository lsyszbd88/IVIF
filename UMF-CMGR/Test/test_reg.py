...
def hyper_args():
    ...
    # dataset
    parser.add_argument('--it',   default='/home/liusy/IVIF/dataset/raw/ctest/Road/it_edge', type=pathlib.Path)
    parser.add_argument('--ir', default='/home/liusy/IVIF/dataset/raw/ctest/Road/ir_w',    type=pathlib.Path)
    parser.add_argument('--disp', default='/home/liusy/IVIF/dataset/raw/ctest/Road/disp',    type=pathlib.Path)
    # checkpoint
    parser.add_argument('--ckpt', default='/home/liusy/IVIF/UMF-CMGR/cache/Reg_only/220507_Deformable_2*Fe_10*Grad/cp_0800.pth', help='weight checkpoint', type=pathlib.Path) # weight/default.pth
    parser.add_argument('--dst',  default='/home/liusy/IVIF/UMF-CMGR/results_Road/Reg/220507_Deformable_2*Fe_10*Grad/', help='fuse image save folder', type=pathlib.Path)
...
def test(net, test_data_loader, dst, device):
    ...
    for (ir, it, disp), (ir_path, it_path, disp_path) in tqdm_loader:
        ...
        # Registration (pad to multiple of 4 for model compatibility)
        _, _, h, w = ir.shape
        pad_h = (4 - h % 4) % 4
        pad_w = (4 - w % 4) % 4
        ...
        start = time.time()
        with torch.no_grad():
            ir_pred, f_warp, flow, int_flow1, int_flow2, disp_pred = net(it_pad, ir_pad, shape=(h + pad_h, w + pad_w))
