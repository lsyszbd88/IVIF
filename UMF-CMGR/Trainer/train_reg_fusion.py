...
# Training settings
    ...
    # dataset
    parser.add_argument('--ir', default=str(DATASET_ROOT / 'raw/ctrain/Road/ir'), type=pathlib.Path)
    parser.add_argument('--vi', default=str(DATASET_ROOT / 'raw/ctrain/Road/vi'), type=pathlib.Path)
    parser.add_argument('--it', default=str(DATASET_ROOT / 'raw/ctrain/Road/it_edge'), type=pathlib.Path)
    parser.add_argument('--ir_map', default=str(DATASET_ROOT / 'raw/ctrain/Road/ir_map'), type=pathlib.Path)
    parser.add_argument('--vi_map', default=str(DATASET_ROOT / 'raw/ctrain/Road/vi_map'), type=pathlib.Path)
    # train loss weights
    ...
    # checkpoint
    parser.add_argument('--load_model_reg', type=str, default=str(CACHE_ROOT / 'Reg_only/220507_Deformable_2*Fe_10*Grad/cp_0800.pth'),
                        help="Location from which any pre-trained model needs to be loaded.")
    parser.add_argument('--load_model_fuse', type=str, default=str(CACHE_ROOT / 'Fusion_only/220507_fusion_w_svm/fus_0200.pth'),
                        help="Location from which any pre-trained model needs to be loaded.")
    # save path of model
    parser.add_argument("--ckpt", default=str(CACHE_ROOT / "Joint_reg_fusion/220508_cotrain_reg_fus"), type=str, help="path to pretrained model (default: none)")
...
def main(args, visdom):
    ...
    for epoch in range(args.start_epoch, args.nEpochs + 1):
        ...
        log.info(dsp)
        print(f"Epoch {epoch:4d} | total={total_loss:.4f}  reg={reg_loss:.4f}  fus={fus_loss:.4f}")
...
if __name__ == "__main__":
    ...
    class DummyVis:
        def images(self, *a, **kw): pass
        def line(self, *a, **kw): pass
    visdom = DummyVis()
    print("[信息] 跳过 Visdom 可视化")
