...
def main(args, visdom):
    ...
    for epoch in range(args.start_epoch, args.nEpochs + 1):
        train(training_data_loader, optimizer, net, criterion, epoch, elastic, affine, visdom)
        if epoch % 20 == 0:
            save_checkpoint(net, epoch, cache)

def train(training_data_loader, optimizer, net, criterion, epoch, elastic, affine, visdom):
    ...
    print(f"Epoch {epoch:4d} | total={loss_avg:.4f}  f-L1={loss_pos:.4f}  ncc={loss_neg:.4f}  grad={loss_grad:.4f}")
