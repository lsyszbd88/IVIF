...
# 将项目根目录 (UMF-CMGR/) 加入 sys.path，确保能导入 functions 等模块
...
def hyper_args():
    ...
    """
    # 获取项目根目录 (UMF-CMGR/)，确保默认路径不受 CWD 影响
    PROJECT_ROOT = pathlib.Path(__file__).resolve().parent.parent

    parser = argparse.ArgumentParser(description='Generating deformable testing data')
    # dataset
    parser.add_argument('--ir',  default=str(PROJECT_ROOT / 'dataset/raw/ctest/Road/ir'),  type=pathlib.Path)
    parser.add_argument('--vi',  default=str(PROJECT_ROOT / 'dataset/raw/ctest/Road/vi'),  type=pathlib.Path)
    parser.add_argument('--dst', default=str(PROJECT_ROOT / 'dataset/test/'), help='fuse image save folder', type=pathlib.Path)
