...
class JointTrainData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 五张图做同位置随机裁剪：用第一张图算参数，全部统一裁剪
        import torchvision.transforms.functional as TF
        _, h, w = ir.shape
        i, j, h_out, w_out = torchvision.transforms.RandomResizedCrop.get_params(
            ir, scale=(0.08, 1.0), ratio=(3./4., 4./3.))
        ir     = TF.resized_crop(ir, i, j, h_out, w_out, (256, 256))
        it     = TF.resized_crop(it, i, j, h_out, w_out, (256, 256))
        vi     = TF.resized_crop(vi, i, j, h_out, w_out, (256, 256))
        ir_map = TF.resized_crop(ir_map, i, j, h_out, w_out, (256, 256))
        vi_map = TF.resized_crop(vi_map, i, j, h_out, w_out, (256, 256))

        return (ir, it, vi, ir_map, vi_map), (str(ir_path), str(it_path), str(vi_path))
